...
export const statusIs = (...status: number[]): Checkers<Response> => ({
  [`status is ${status.sort().join(' or ')}`]: (r: Response) =>
    status.includes(r.status),
})

export const mergeParams = (
  params1: Params | null | undefined,
  params2: Params | null | undefined
): Params => {
  return {
    ...params1,
    ...params2,
...
export function makeRequestTemplate<TUrl extends string>(
