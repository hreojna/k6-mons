...
export class HttpSession {
  ...
  async asyncBatch(requests: Request[]): Promise<Response[]> {
    return Promise.all(
      requests.map((request: Request) => this.asyncSend(request))
    )
  }
  ...
  check(
    ...
    if (checks && Object.keys(checks).length > 0) {
      ...
      if (logError && !result && this.option.errorHandler)
        this.option.errorHandler.logError(response, attempt)
      if (failError && !result)
        if (attempt === 1)
          fail(
            `Request failed: ${response.request.method} ${response.request.url}`
          )
        else
          fail(
